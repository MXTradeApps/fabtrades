...
- Seed: if missing, insert Trade Binder (`Trade Binder`, `role=trade`) and Collection (`Collection`, `role=standard`). After first persist, never resurrect Collection; always restore Trade Binder if absent or tombstoned.

Grid order: Trade Binder, Collection if live (`clientId = system:collection`), then other live Binders by `createdAt` ascending, then `clientId`.
...
| `client_id` (Postgres / sync) | `binder|{binderId}|{cardId}|{condition}` or `want|{cardId}` |
...
Migration: every existing `is_wanted = false` row gets `binder_id = system:trade` and keeps its condition. Want List rows stay `binder_id` null. Mobile load of legacy `collection_entries` JSON missing `binder_id` assigns Trade Binder.
...
See [contracts/binder-move.md](./contracts/binder-move.md).
...
Pro: both Binder caps removed (existing “Pro removes all of them”). See [contracts/free-limits.md](./contracts/free-limits.md).
...
Migration: `supabase/migrations/20260822194254_multi_binders.sql`.

`public.binders`:

- `id UUID PK`, `user_id`, `client_id`, `name`, `role` (`trade` \| `standard`), `created_at`, `updated_at`, `deleted_at`
...
- `client_id TEXT NOT NULL` — sync identity (see above)
- Drop `UNIQUE (user_id, card_id, is_wanted)`
- `UNIQUE (user_id, client_id)` for upsert including tombstones
...
Backfill: upsert `system:trade` and `system:collection` for users who have binder entries **or** a binder share; set owned `binder_id` to `system:trade`; fill `client_id` from identity. `private.get_public_binder_by_token` returns owned live rows whose Binder `role = trade` only.

## Sync

New domain `binders` (client_id identity, tombstones). Binder entries domain identity becomes `binder|{binderId}|{cardId}|{condition}` vs `want|{cardId}`. Conflict target `(user_id, client_id)`. Last write wins per record (`docs/CLOUD_SYNC.md`). Unique-violation on a live Binder name is a surfaced error, not a silent rename.
