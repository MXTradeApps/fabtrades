...
Touches **I** (grid + two defaults + move + limits; no folders / share-all-Binders / Collection-in-Confirm-Trade), **II** (`role=trade` vs display name; Want List is `is_wanted`, not a Binder; cover/value derived), **III** (duplicate name refused; Trade Binder delete refused; non-empty delete refused; unique-violation on sync is a surfaced error, not a silent rename; unpriced tile is unpriced, not $0), **IV** (extend `free_limits.json`; add `binder_move.json` / `binder_cover.json` / `binder_names.json`; both suites assert them), **V** (pipeline unchanged; covers use catalog `image_url`). Vocabulary is already constitution **1.1.0** and `docs/CONTEXT.md`: Collection is a default Binder **name**; do not rename the Binder destination. Also: table is the deadline (Confirm Trade / filler stay Trade Binder), no gate before value (signed-out mobile), one brand two peer surfaces, real prices or nothing, local reads, dual-client DRY (fixtures not a runtime), server-owned Pro (5th Binder paywall does not write `entitlements`).
...
**Purpose**: Schema, Binder records, entry identity, seed/migration, sync, shared helpers, and Trade Binder scoping. Grid stories only display these objects. Constitution 1.1.0 and `docs/CONTEXT.md` already allow Collection as a Binder name — do not amend them again.
...
**Independent Test**: Open the Binder destination as a new player (empty stock) and as a player who already had Binder cards. Confirm the grid shows Trade Binder and Collection, each with name, card count, and value. Non-empty Trade Binder shows a cover of its highest-value card; empty Collection shows no cover. Open Trade Binder, then return to the grid. Existing Binder cards appear in Trade Binder, not Collection. Signed-out mobile: grid and lists without creating an account. Web signed-in `/binder` matches; do not invent signed-out web Binder.
...
**Independent Test**: Put a Printing with quantity 3 in Trade Binder. Move 2 copies to Collection. Confirm Trade Binder shows 1 remaining, Collection shows 2, and both tiles' counts and values match the lists. Destination already having that Printing+condition merges. Want List is not offered as a destination.
...
**Independent Test**: Create a Binder named "Side Event", add no cards, rename it, then delete it. Attempt to delete Trade Binder and confirm the product refuses. Confirm Trade Binder is still on the grid. Free player with 4 Binders: 5th create shows Pro upgrade and still has exactly 4. Duplicate names (including `collection` vs `Collection`) refused.
...
- Schema + Binder records + entry `binderId` + `client_id` (Phase 2) before any grid
- Cover helper (Phase 2) before tile images
- `openBinderIdProvider` / `openBinder.js` (Phase 2 / US1) before add-target wiring
...
- T006, T008, T009 in parallel (docs + models; T007 SQL can run beside them)
- T010 and T012 in parallel after T008
- T011 after T009; T013 after T012
- T015 in parallel with mobile repos (web `binder.js`)
- T016 and T017 in parallel; T018 and T019 after their failing tests
- T020 and T021 in parallel; T022 and T023 after their failing tests
- T024, T025, T026, T027 in parallel (mobile confirm / web confirm / filler / share)
- T028 and T029 in parallel; T030 and T031 in parallel; T035 in parallel with grid chrome
- T040 and T041 in parallel; T043 in parallel with T042
...
- T057 and T058 in parallel; T063 and T064 in parallel (coordinate `providers.dart` / `binder.js`)
...
# Add-target helper (web) beside grid:
Task: "openBinder.js in apps/web/src/utils/openBinder.js"
```
...
2. Complete Phase 2: Foundational (schema `20260822194254_multi_binders.sql`, seed/migrate, sync, cover, free-limit helpers, Trade Binder scoping)
...
- Constitution 1.1.0 already allows Collection as a Binder name; do not rename the Binder tab
