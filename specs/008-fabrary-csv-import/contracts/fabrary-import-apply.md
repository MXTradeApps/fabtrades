...
Cap numbers stay in `packages/contracts/free_limits.json`. This fixture describes **batch** behaviour (refuse-all vs add-on-top). Both suites assert both files.
...
7. Free player: incoming distinct that would exceed 50 → `free_cap`, empty adds.
8. Free player: incoming printings already owned (any Binder) → allowed; distinct count unchanged.
9. Pro: large incoming set allowed.
10. Zero matched owned rows → `no_matched`.
11. No Have>0 → `no_owned`.
12. Missing required headers → `not_fabrary`.

## Apply (after confirm)

Callers MUST:

1. Refuse if `!ok` (including `free_cap` at confirm).
...
```text
if isPro → allow
resultingDistinct = |existingOwnedIds ∪ incomingIds|
allow iff resultingDistinct ≤ FreeLimits.binderCards
```

Add these cases to `free_limits.json` (do not fork a second cap number).
